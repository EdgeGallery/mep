...
RUN GOOS=linux go build -buildmode=pie -ldflags '-linkmode "external" -extldflags "-static"' -o healthcheck-m
...
COPY --from=builder --chown=$USER_NAME:$GROUP_NAME $HOME/healthcheck-m $HOME/bin
